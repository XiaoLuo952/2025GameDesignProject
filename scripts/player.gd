...
func _physics_process(delta: float) -> void:
	...
	# 跳跃逻辑 - 地面跳跃或墙壁跳跃
	if Input.is_action_just_pressed("ui_accept") and (is_on_floor() or is_climbing) and not is_dashing:
		velocity.y = JUMP_VELOCITY
		is_climbing = false  # 跳跃时停止攀爬
	...
	if is_dashing:
		...
		if dash_timer <= 0:
			...
			# 保留一部分动量，使过渡更自然
			velocity = velocity * DASH_EXIT_SPEED
			
			# 如果是向上冲刺，给一个合理的上升速度而不是直接归零
			if dash_direction.y < 0:
				velocity.y = min(velocity.y, JUMP_VELOCITY * 0.7)  # 类似于一个小跳的感觉
		else:
			# 冲刺期间持续应用速度，确保方向保持一致
...
func start_dash(direction: Vector2):
	...
	dash_direction = direction.normalized()  # 保存规范化的方向
	velocity = dash_direction * DASH_SPEED   # 设置初始冲刺速度
...
func set_dash_count(count: int) -> void:
	dash_count = min(count, MAX_DASH_COUNT)  # 确保不超过最大值
